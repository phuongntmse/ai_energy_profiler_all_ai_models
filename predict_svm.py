...
for file_name in os.listdir(TESTING_DATA_PATH):
	if file_name.endswith(".csv"):
		...
		for group, features in FEATURE_GROUPS.items():
			...
			try:
				model = joblib.load(f"{MODEL_DIR}/model_svm_{group}_{SVM_KERNEL}_{SVM_GAMMA}_{SVM_C}_{HW_INFRAS_FOR_TRAINING_DATA}.pkl")
				scaler = joblib.load(f"{MODEL_DIR}/scaler_svm_{group}_{SVM_KERNEL}_{SVM_GAMMA}_{SVM_C}_{HW_INFRAS_FOR_TRAINING_DATA}.pkl")
				expected_cols = joblib.load(f"{MODEL_DIR}/metrics_{group}_{SVM_KERNEL}_{SVM_GAMMA}_{SVM_C}_{HW_INFRAS_FOR_TRAINING_DATA}.pkl")
